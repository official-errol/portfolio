...
export const CommentSection: React.FC<{ postId: string }> = ({ postId }) => {
  ...
  const handleReaction = async (commentId: string, type: 'like' | 'dislike') => {
    ...
    if (existing) {
      if (existing.reaction_type === type) {
        ...
        await supabase.from('comment_reactions').update({ reaction_type: type }).eq('comment_id', commentId).eq('user_id', user.id)
        setReactions(reactions.map(r => r.comment_id === commentId && r.user_id === user.id ? { ...r, reaction_type: type } : r))
      }
    } else {
  ...
  const renderComment = (comment: Comment) => {
    ...
    return (
      <div key={comment.id} className={`${comment.parent_id ? 'border-t pt-3' : 'border-b'} border-gray-200`}>
          ...
          <div className="flex-1">
            ...
            {user && (
              <div className="flex gap-6 mt-2 text-sm items-center text-gray-500">
                ...
                <button onClick={() => setReplyTo(comment.id)} className="flex items-center gap-1 hover:underline">
        ...
        <div className="ml-10 mt-2 space-y-3">
          {comments.filter(r => r.parent_id === comment.id).map(reply => renderComment(reply))}
        </div>
      ...
      {user ? (
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button onClick={() => redirectToLogin('google')} className="flex items-center gap-3 px-4 py-2 bg-white border rounded hover:shadow">
              <img src="https://cdn.freebiesupply.com/logos/large/2x/google-icon-logo-png-transparent.png" alt="Google" className="w-5 h-5" />
              Sign in with Google
            </button>
            <button onClick={() => redirectToLogin('github')} className="flex items-center gap-3 px-4 py-2 bg-black text-white rounded hover:bg-gray-800">
